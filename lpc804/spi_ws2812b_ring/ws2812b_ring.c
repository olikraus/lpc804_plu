...
volatile uint16_t spi_pos = 0;		// current position/next in spi_data 
volatile uint16_t spi_max = 0;		// size of the data in spi_data

void __attribute__ ((interrupt)) SPI0_Handler(void)
{
  //uint32_t active = LPC_SPI0->STAT & LPC_SPI0->INTENSET;    // ignored, there is only a TXRDY interrupt
  if ( spi_pos < spi_max )
  {
    LPC_SPI0->TXDAT = spi_data[spi_pos];
    spi_pos++;
  }
  else
  {
...
void spi_out(uint8_t *data, int cnt)
{
  ...
  spi_pos = 1;
  spi_max = cnt;
  LPC_SPI0->TXDAT = data[0];	/* load first byte, so that the TXRDY interrupt will be generated */
